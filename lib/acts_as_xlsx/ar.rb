  ...
  module Ar
    ...
    module SingletonMethods
      ...
      def to_xlsx(options = {})
        ...
        sheet_name = options.delete(:name) || (i18n ? I18n.t("activerecord.models.#{table_name.singularize.underscore}.other") : table_name.humanize) 
        ...
        p.workbook.add_worksheet(:name=>sheet_name) do |sheet|
          ...
          data.each do |r|
            row_data = columns.map do |c|
              if c.to_s =~ /\./
                v = r; c.to_s.split('.').each { |method| v = v.try(method) }; v
              else
                r.send(c)                
              end
            end
